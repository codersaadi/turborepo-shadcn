...
// Types
type IconType = React.ForwardRefExoticComponent<React.SVGProps<SVGSVGElement> & { size?: string | number }>;
type LogoProps = {
    extra?: string;
    size?: number;
    className?: string;
...
const CustomTextLogo = memo<{ size: number; style?: React.CSSProperties; className?: string }>(
...
const CustomImageLogo = memo<{ size: number; style?: React.CSSProperties; className?: string }>(
    ({ size, style, className }) => {
        return (
            <Image
                alt={BRANDING_NAME}
...
const CustomLogo = memo<LogoProps>(({ extra, size = 32, className, style, type, ...rest }) => {
    ...
    return (
        <div className={cn("flex items-center flex-none", className)} {...rest}>
            ...
            <div className="font-light whitespace-nowrap" style={{ fontSize: extraSize }}>
                {extra}
            </div>
        </div>
    );
